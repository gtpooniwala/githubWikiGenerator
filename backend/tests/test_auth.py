...
def test_wrong_key_returns_401():
    response = client.post(GENERATE_URL, json=VALID_BODY, headers={"x-api-key": "wrong-key"})
    assert response.status_code == 401


def test_correct_key_returns_200():
    response = client.post(GENERATE_URL, json=VALID_BODY, headers={"x-api-key": VALID_KEY})
